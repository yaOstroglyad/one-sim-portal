...
CMD ["/bin/sh", "-c", "envsubst '$MAIN_BACKEND_SERVER $MAIN_BACKEND_HOST $API_PRODUCT_SERVER $API_TICKETING_SERVER $API_PRODUCT_HOST' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/conf.d/default.conf && nginx -g 'daemon off;'"]

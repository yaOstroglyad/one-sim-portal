...
export class OsLineChartComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  @Input() smooth: boolean = true; // Enable/disable bezier curves
  ...
  private readonly defaultOptions: LineChartOptions = {
    responsive: true,
    ...
    elements: {
      line: {
        tension: 0.4, // Default smooth curves
        borderWidth: 2
      },
      point: {
        radius: 4,
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['smooth'] && !changes['smooth'].firstChange) {
  ...
  private initializeChart(): void {
    if (!this.lineChartCanvas?.nativeElement || !this.data) {
      return;
    }
  ...
  private applySmoothnessSetting(options: LineChartOptions): void {
    if (options.elements?.line) {
      options.elements.line.tension = this.smooth ? 0.4 : 0;
    }
  }
  ...
  private applyThemeToData(): void {
    ...
    this.data.datasets.forEach((dataset, index) => {
      if (!dataset.backgroundColor) {
        ...
        dataset.backgroundColor = this.addOpacity(baseColor, 0.1);
      }
      if (!dataset.borderColor) {
        dataset.borderColor = this.getDefaultColor(index);
      }
      ...
      if (dataset.tension === undefined && this.smooth) {
        dataset.tension = 0.4;
      }
      if (dataset.fill === undefined) {
        dataset.fill = false; // Default no fill for line charts
      }
    });
  }
  ...
  private mergeOptions(defaultOptions: LineChartOptions, userOptions: LineChartOptions): LineChartOptions {
    // Simple merge for Chart.js options, avoiding deep type conflicts
    const merged: LineChartOptions = {
      ...defaultOptions,
      ...userOptions
    };
    ...
    // Merge elements if both exist
    if (defaultOptions.elements && userOptions.elements) {
      merged.elements = {
  ...
  public downloadChart(filename: string = 'line-chart', format: 'png' | 'jpeg' = 'png'): void {
  ...
  public addDataPoint(label: string, dataPoints: number[]): void {
    if (!this.chart || !this.data) {
      return;
    }

...
describe('UserListComponent', () => {
  ...
  let mockRoleService: jest.Mocked<Partial<RoleService>>;
  let mockAuthService: jest.Mocked<Partial<AuthService>>;
  let mockTableService: jest.Mocked<Partial<UsersTableService>>;
  let mockUserRoleService: jest.Mocked<Partial<UserRoleService>>;
  let mockCompaniesDataService: jest.Mocked<Partial<CompaniesDataService>>;
  ...
  beforeEach(async () => {
    ...
    mockUserService = {
      paginatedUsers: jest.fn().mockReturnValue(of(mockPaginatedResponse)),
      getUserTypes: jest.fn().mockReturnValue(of(['CORPORATE', 'PRIVATE'])),
      deleteUser: jest.fn().mockReturnValue(of(undefined)),
      assignRoles: jest.fn().mockReturnValue(of(undefined)),
      removeRoles: jest.fn().mockReturnValue(of(undefined))
    };
    ...
    mockTableService = {
      getTableConfig: jest.fn().mockReturnValue(new BehaviorSubject(mockTableConfig)),
      updateConfigData: jest.fn(),
      updateColumnVisibility: jest.fn(),
      setUserRolesTemplate: jest.fn()
    };
  ...
  describe('user actions', () => {
    ...
    it('should show error notification on delete failure', async () => {
      // Arrange
      mockUserService.deleteUser = jest.fn().mockReturnValue(
        new (await import('rxjs')).Observable(subscriber => subscriber.error(new Error('Delete failed')))
      );
      component.selectedUser.set(mockUsers[0]);
  ...
  describe('filters', () => {
    ...
    it('should load data on filter change', async () => {
      ...
      // Assert
      expect(mockUserService.paginatedUsers).toHaveBeenCalledWith(
        expect.objectContaining({ searchQuery: 'test', type: 'CORPORATE' }),
        0,
        15
      );
    });
  ...
  describe('role management permissions', () => {
    it('should return true for canManageRoles when user is admin', () => {
      // Arrange
      mockUserRoleService.isAdmin = jest.fn().mockReturnValue(true);
    ...
    it('should return false for canManageRoles when user is not admin', () => {
      // Arrange
      mockUserRoleService.isAdmin = jest.fn().mockReturnValue(false);
  ...
  describe('role assignment', () => {
    ...
    it('should filter out ADMIN role and already assigned roles', async () => {
      // Arrange
      mockRoleService.getRoles = jest.fn().mockReturnValue(of({
        content: [
          { id: 'role1', name: 'USER', displayName: 'User' },

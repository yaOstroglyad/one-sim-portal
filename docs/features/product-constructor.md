...
## Company Product Pricing Schedule

### Overview

Company Products support multiple prices with different `validFrom` dates, allowing scheduled price changes.

### Component

**Location:** `src/app/views/product-constructor/components/company-products/company-product-prices-table/`
...
```typescript
interface CompanyProductPrice {
  id: string;
  companyProductId: string;
  tariffOfferId: string;
  price: number;
  currency: Currency;  // 'USD' | 'EUR' | 'GBP'
  validFrom: string;   // ISO date: '2025-11-20'
}
```
...
| Rule | Description |
|------|-------------|
| **Current Price** | Price with `validFrom <= today` and closest to today |
| **Future Price** | Price with `validFrom > today` (editable) |
| **Past Price** | Price with `validFrom < today` (read-only) |
| **Edit Protection** | Only future prices can be edited |
| **No Duplicates** | `validFrom` dates must be unique per product |

### Validation Utils

**Location:** `company-product-prices-table/utils/price-validation.utils.ts`

```typescript
class PriceValidationUtils {
  // Check if price can be edited (future only)
  static isFuturePrice(validFrom: string): boolean;

  // Get minimum valid date for new price
  static getMinValidFromDate(tariffOfferValidFrom: string): string;

  // Check for duplicate validFrom dates
  static hasValidFromConflict(validFrom, existingPrices, excludePriceId?): boolean;

  // Find current active price
  static getCurrentPrice(prices: CompanyProductPrice[]): CompanyProductPrice | null;

  // Check if price is the current one
  static isCurrentPrice(price, allPrices): boolean;
}
```
...
| State | Badge | Actions |
|-------|-------|---------|
| Current | `Current` (green) | View only |
| Scheduled | `Scheduled` (blue) | Edit, Delete |
| Past | - | View only |

### Usage

```html
<app-company-product-prices-table
  [prices]="prices()"
  [loading]="isLoading()"
  [editable]="canEdit"
